...
def register():
    ...
    try:
        ...
        active_campaigns = Campaign.query.filter_by(status='Active').with_for_update().all()
...
def edit_campaign(campaign_id):
    ...
    flash(
        ...
        f'Duration: {new_duration_months} month(s). Use "Allocate Tasks" to assign the new slots.',
        'success'
    )
    return redirect(url_for('admin_campaigns'))
...
def allocate_tasks(campaign_id):
    ...
    try:
        ...
        progress = db.session.query(CampaignAllocationProgress).with_for_update().filter_by(campaign_id=campaign_id).first()
        if not progress:
        ...
        if campaign_cap and planned >= campaign_cap:
            return jsonify({'status': 'error', 'message': 'Campaign task limit has already been reached.'}), 400
        allowed_to_plan = campaign_cap - planned if campaign_cap else total_tasks_needed
        tasks_to_plan = min(total_tasks_needed, allowed_to_plan)
        if tasks_to_plan <= 0:
            return jsonify({'status': 'error', 'message': 'No task slots remain in this campaign.'}), 400
        ...
        assigned_this_month = db.session.query(UserCampaignTaskAssignment.user_id).filter(
            UserCampaignTaskAssignment.campaign_id == campaign_id,
        ...
        return jsonify({'status': 'success', 'message': msg, 'assigned': actual, 'pending': pending})
    except SQLAlchemyError as e:
        db.session.rollback()
