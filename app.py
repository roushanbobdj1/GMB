...
with app.app_context():
    ...
    try:
        ...
        # ✅ FIX: 'tasks.points_per_review' column (campaign delete hone ke
        # baad bhi task ke points sahi dikhein — 0 points bug fix). Missing
        # ho to add karo, aur purane tasks ke liye ek baar campaign se
        # points backfill bhi kar do.
        if 'tasks' in inspector.get_table_names():
            existing_task_cols = [c['name'] for c in inspector.get_columns('tasks')]
            if 'points_per_review' not in existing_task_cols:
                with db.engine.connect() as conn:
                    conn.execute(text('ALTER TABLE tasks ADD COLUMN points_per_review INTEGER DEFAULT 0'))
                    conn.commit()
                logger.info("✅ Added missing column 'points_per_review' to tasks table.")

                # Backfill: existing tasks ke liye unki (abhi bhi active)
                # campaign se points copy kar do.
                with db.engine.connect() as conn:
                    conn.execute(text('''
                        UPDATE tasks
                    ...
                    ''') if db.engine.dialect.name != 'sqlite' else text('''
                        UPDATE tasks
                        SET points_per_review = (
                    ...
                    '''))
                    conn.commit()
                logger.info("✅ Backfilled points_per_review for existing tasks.")
...
def register():
    ...
    # ✅ DIRECT REGISTRATION (OTP TEMPORARILY DISABLED)
    try:
        # Create User
...
def view_tasks():
    ...
    # hai) sabse upar, phir Submitted (pending review), phir Approved (done)
    # sabse neeche — taki purane complete tasks ke bich naya kaam na dab jaye.
...
@login_required
def clear_tasks():
    """✅ Naya feature: user apne Approved/Rejected (complete/history) tasks
    ki list se clear kar sake, taki mahino baad list chhoti/manageable rahe.
    Assigned/Submitted (jo abhi pending hai) kabhi clear nahi hote."""
    user_id = session.get('user_id')
    deleted = (
        Task.query
        .filter(Task.user_id == user_id, Task.status.in_(['Approved', 'Rejected']))
...
def delete_campaign(campaign_id):
    ...
    # ✅ FIX: Sirf incomplete (Assigned) assignments delete karo — jo tasks
    # user complete/submit/reject kar chuka hai unki history (assignment
    # record) preserve rahegi, sirf campaign se unlink hogi.
    UserCampaignTaskAssignment.query.filter_by(
        campaign_id=campaign.id, status='Assigned'
    ).delete()
    UserCampaignTaskAssignment.query.filter_by(campaign_id=campaign.id).update(
        {'campaign_id': None}
    )
...
def task_verify_action(submission_id, action):
    ...
    try:
        if action == 'approve':
            ...
            # ✅ FIX: campaign delete ho chuka ho to task.campaign None hota hai —
            # ab snapshot ke points_per_review se award hoga (crash-safe).
            points = task.points_per_review or (task.campaign.points_per_review if task.campaign else 0)
